...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  List<Widget> build(List<md.Node> nodes) {
    ...
    for (final node in nodes) {
      assert(_blocks.length == 1, 'More than one root element found.');
      node.accept(this);
    }

    assert(_tables.isEmpty, 'Unclosed table found.');
    assert(_inlines.isEmpty, 'Unclosed inline element found.');
    assert(!_isInBlockquote, 'Unclosed blockquote found.');
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (_isListTag(tag)) {
        assert(_listIndents.isNotEmpty, 'List indents should not be empty');
  ...
  Widget _buildImage(String src, String? title, String? alt) {
    ...
    if (sizedImageBuilder != null) {
      final config = MarkdownImageConfig(uri: uri, alt: alt, title: title, height: height, width: width);
